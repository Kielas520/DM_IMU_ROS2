...
# 使用你的串口实现（保持路径）
from dm_imu.models.dm_serial import DM_Serial
...
class DmImuNode(Node):
    def __init__(self):
        ...
        self.publish_rpy_in_degree = False
        ...
        self.pub_pose = self.create_publisher(PoseStamped, 'imu/pose', 10)
    ...
    def _on_timer_publish(self):
        ...
        ok, stamp_ts, r_deg, p_deg, y_deg = self._extract_latest(latest)
        if not ok:
